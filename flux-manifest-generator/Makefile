...
IMG ?= tvanderpool/flux-manifest-generator:latest
...
# Track the downloaded source-controller CRD version
$(SOURCE_CRD_VER):
...
# Download the CRDs the controller depends on (used for envtest)
...
deploy: manifests
	cd config/manager && kustomize edit set image flux-manifest-generator=${IMG}
...
# Run go tidy to cleanup go.mod for both modules
tidy:
...
# Remove generated binaries, envtest assets, and coverage reports.
# Generated source files (zz_generated*.go, config/crd/bases/*.yaml)
# are intentionally left in place since they are part of the committed
# source tree.
.PHONY: clean
clean:
	rm -rf $(REPOSITORY_ROOT)/flux-manifest-generator/bin/data
	rm -f $(REPOSITORY_ROOT)/flux-manifest-generator/cover.out
	# envtest unpacks read-only binaries; widen perms so rm -rf succeeds.
	[ ! -d $(REPOSITORY_ROOT)/flux-manifest-generator/testbin ] || \
	  chmod -R u+w $(REPOSITORY_ROOT)/flux-manifest-generator/testbin
	rm -rf $(REPOSITORY_ROOT)/flux-manifest-generator/testbin
